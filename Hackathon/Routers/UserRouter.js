...
UserRouter.get("/", async(req, res) => {
    try {
        const users = await UserModel.find({}, "name email hashPassword avatar intro goal weight height");
...
UserRouter.post("/", async(req, res) => {
    const {name, email, password, avatar, intro, goal, weight, height} = req.body;
    const hashPassword = bcrypt.hashSync(password, bcrypt.genSaltSync());
    try {
        const userCreated = await UserModel.create({name, email, hashPassword, avatar, intro, goal, weight, height});
...
UserRouter.put("/:id", async (req, res) => {
    var userId = req.params.id;
    const {name, password, avatar, intro, goal, weight, height} = req.body;
    try {
        const userFound = await UserModel.findById(userId);
        if(!userFound){
            res.status(404).json({success: 0, message: "Not Found!"})
        } else{
            for(key in {name, password, avatar, intro, goal, weight, height}){
